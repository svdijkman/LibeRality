test_that("design GUI retains shared theme and accessible dialogs", {
  ...
  expect_match(css, "background:linear-gradient(105deg,#f6ca78,#dfa13e)", fixed = TRUE)
  expect_match(css, ".ly-dark .ly-header{background:#c58a32;color:#2e1e0a", fixed = TRUE)
  expect_match(css, ".ly-brand strong{font-size:19px;letter-spacing:.2px;color:#2e1e0a}", fixed = TRUE)
  expect_match(css, ".ly-dark .ly-switch input:checked+i{background:var(--ly-accent)}", fixed = TRUE)
  expect_match(css, ".ly-dark .ly-header .ly-badge-warning{color:#fff3df}", fixed = TRUE)
